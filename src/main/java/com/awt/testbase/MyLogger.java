...
/**
 * By this class we are initializing the logger object
 * @author Ankit Yadav
 */
public class MyLogger {
	...
	public static synchronized void startTestCase(String sTestCaseName) {
		...
		startLog(System.getProperty("user.dir") + "/TestResult/Logs", sTestCaseName);
	...
	/**
	 * By this method if we can get same log file so we can create one another file
	 * by incrementing the number so log file can't be override to each other
	 *
	 * @param dirPath
	 * @param testCaseName
	 * @author Ankit Yadav
	...
	public static String getCallInfo() {
		...
		//callInfo = className + ":" + methodName + ":" + " Line Number Is :" + lineNumber + " ";
		callInfo = methodName + ":" ;
		return callInfo;
